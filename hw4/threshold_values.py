...
def threshold_values(seq, threshold=1):
    '''
        Keep bitstrings with the number of highest frequency indicated by threshold
        only the top threshold bitstrings keep value of 1
        seq: list of bitstrings
        threshold: integer
        return: dictionary of key as bitstring, value is from map_bistrings
    '''
    assert isinstance(seq, list)
    assert isinstance(threshold, int)
    assert threshold > 0
    
    gathVals = gather_values(seq)
    print(gathVals)
    sortgathVals = dict(sorted(gathVals.items(), key=lambda item: (len(item[1]),-int(item[0], 2)), reverse=True))
    print(sortgathVals)
    for k in sortgathVals:
        if sortgathVals[k][0] == 0:
            sortgathVals[k] = 0
        else:
            if threshold == 0:
                sortgathVals[k]= 0
